...
function generateMQpar() {
	...
	#add real files
	grep $MSPLATE $MAPPING | grep ",$NAME," | awk -v EXT="$RAW_FILE_EXT" -F "," '{print $1"."EXT","$2","$3","$4","$5","$6","$7","$8}' > $LMAPPING
	...
	ACTIVE_BASE_MQPAR="$ROOT_WD/Queue/Maxquant/$ALT_BASE_MQPAR"
...
function cpRaws() {
	...
	for file in `cat $LMAPPING | grep ",$NAME," | awk -F ',' -v x=$RAWFILE_POSITION '{print $x}'`; do
		if [ ! -e `basename $file` ]; then
