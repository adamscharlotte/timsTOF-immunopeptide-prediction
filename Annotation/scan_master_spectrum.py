...
def binning(inp, ignoreCharges):
    ...
    ms.export_to_csv(base_path + "total-scan-consensus/tmp-MasterSpectrum/24102022.csv")
    comb_ms = pd.read_csv(base_path + "total-scan-consensus/tmp-MasterSpectrum/24102022.csv")
...
un_annot_df_combined["OBS_SEQUENCE"] = internal_without_mods(un_annot_df_combined["MODIFIED_SEQUENCE"])
un_annot_df_combined["PEPTIDE_LENGTH"] = un_annot_df_combined["OBS_SEQUENCE"].apply(lambda x: len(x))
...
un_annot_df_combined["OBS_SEQUENCE_INT"] = array.tolist()

# ------------- Sequence int full length sequence -------------
generator_sequence_numeric = parse_modstrings(list(un_annot_df_combined["SEQUENCE"].values), ALPHABET, translate=True)
enum_gen_seq_num = enumerate(generator_sequence_numeric)
array = np.zeros((len(list(un_annot_df_combined["SEQUENCE"].values)),30), dtype=np.uint8)
for i, sequence_numeric in enum_gen_seq_num:
    if len(sequence_numeric) > 30:
        if filter:
            pass # don"t overwrite 0 in the array that is how we can differentiate
