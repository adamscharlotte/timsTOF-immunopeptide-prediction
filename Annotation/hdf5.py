...
test_df.INTENSITIES = test_df.INTENSITIES.str.split(";").apply(lambda s: [float(x) for x in s])
test_df.MZ = test_df.MZ.str.split(";").apply(lambda s: [float(x) for x in s])
test_df.OBS_SEQUENCE_INT = test_df.OBS_SEQUENCE_INT.str.strip("][").str.split(", ").apply(lambda s: [int(x) for x in s])
test_df.SEQUENCE_INT = test_df.SEQUENCE_INT.str.strip("][").str.split(", ").apply(lambda s: [int(x) for x in s])
test_df['PRECURSOR_CHARGE'] = test_df['PRECURSOR_CHARGE'].astype(int)
...
test_df["PRECURSOR_CHARGE_ONEHOT"] = test_df.apply(lambda row: int_to_onehot(row.PRECURSOR_CHARGE), axis = 1)
...
collision_energies = np.asarray(test_df.ORIG_COLLISION_ENERGY.values)
adjusted_collision_energies = np.asarray(test_df.aligned_collision_energy.values/100.0)
collision_energies_normalized = np.asarray(test_df.ORIG_COLLISION_ENERGY.values/100.0)
intensities = np.asarray(test_df.INTENSITIES.values.tolist())
masses = np.asarray(test_df.MZ.values.tolist())
methods = np.asarray(test_df.FRAGMENTATION.values,dtype ='S3')
precursor_charges = np.asarray(test_df.PRECURSOR_CHARGE_ONEHOT.values.tolist())
raw_files = np.asarray(test_df.RAW_FILE.values,dtype='S120')
scan_numbers = np.asarray(test_df.SCAN_NUMBER.values)
scores = np.asarray(test_df.SCORE.values)
sequence_integers = np.asarray(test_df.SEQUENCE_INT.values.tolist())
obs_sequence_integers = np.asarray(test_df.OBS_SEQUENCE_INT.values.tolist())
retention_time = np.asarray(test_df.median_RETENTION_TIME.values)

if os.path.exists(hdf_test_path):
    with h5py.File(hdf_test_path, 'a') as hf:
        hf["collision_energy"].resize((hf["collision_energy"].shape[0] + collision_energies.shape[0]), axis = 0)
        hf["collision_energy"][-collision_energies.shape[0]:] = collision_energies
