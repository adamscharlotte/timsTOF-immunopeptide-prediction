...
install.packages('timsr') #, lib = "/home/cadams/R/x86_64-pc-linux-gnu-library")
...
# setwd()
path="/media/c2m/5_user_files/cadams/2022-library-run/20220621_AspNlysN/AspNLysN-A5_S1-A5_1_6546.d"
# path = '/Users/adams/Projects/300K/20220613-library-test/raw/E1_50fmol_bruker_S1-E2_1_6339.d'
...
tbl_all <- D[c(1949, 1950), all_columns] %>% as_tibble
tbl_all %>% filter(scan<878 & scan>850) %>% print(n=200) %>% arrange(mz) %>% pull(mz) 
...
D[2590, all_columns] %>% as_tibble %>% select(scan, tof) %>% arrange(scan)
D[2591, all_columns] %>% as_tibble %>% select(scan, tof) %>% arrange(scan)
D[2592, all_columns] %>% as_tibble %>% select(scan, tof) %>% arrange(desc(scan))


D[6524, all_columns] %>% as_tibble %>% pull(retention_time) %>% unique()
D[c(6487, 6494:6496), all_columns] %>% as_tibble  %>% pull(retention_time) %>% unique()

%>% filter(scan == 1025) %>% arrange(intensity)


D[1317, all_columns] %>% as_tibble
 %>% pull(retention_time) %>% unique()
D[c(3462, 3472), all_columns] %>% as_tibble  %>% pull(retention_time) %>% unique()

D[c(3462, 3472), all_columns] %>% as_tibble


 %>% filter(retention_time == 451.076)
