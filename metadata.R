...
fwrite(tbl_pool_names, pool_name_output_path, col.names=F)

tbl_meta_length <- tbl_meta %>% mutate(length = str_count(Sequence))    # %>% select(length) %>% arrange(length) %>% unique()
tbl_meta_minmax <- tbl_meta_length %>% 
	mutate(pool_name=`Pool name`) %>%
	group_by(`Pool name`) %>% 
	mutate(min_length = min(length), max_length = max(length)) %>% 
	ungroup()
fwrite(tbl_meta_minmax, meta_output_path, col.names=T, sep="\t")
...
list_meta <- list_files[grep(pattern = "meta", list_files)]#list_files[-grep(pattern = "location-map", list_files)]
...
tbl_folder_map <- merge(tbl_map, tbl_folder_names) %>% as_tibble %>% filter(str_detect(folder_names, paste(location, "_", sep="")))
...
fwrite(tbl_pool_sequence, full_pool_sequence_path, col.names=T, sep="\t")

# ------------------------------------------------------------------------------------------------------------------------------------------------------------
# Some folders are missing. I assume these were empty wells that were analyzed anyway. We could test
# to see whether we identify anything, but I think we won't.
tbl_folder_names %>% filter(!folder_names %in% tbl_folder_map$folder_names) %>% print(n=60) 
# ------------------------------------------------------------------------------------------------------------------------------------------------------------

tbl_folder_meta_map <- merge(tbl_folder_map, tbl_meta, by="pool_name") %>% as_tibble %>% select(-c(length, Sequence)) %>% unique()
fwrite(tbl_folder_meta_map, full_meta_map_path, col.names=T, sep="\t")
