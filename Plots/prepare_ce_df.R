...
ce_output_path <- paste(base_path, "Annotation/QC-collission-energy.csv", sep = "")
...
tbl_qc_ce <- tbl_mapped_precursor %>%
    filter(stringr::str_starts(Proteins, "QC")) %>%
    mutate(pool_name = pool) %>%
    select(pool_name, Scan_number, Proteins, Sequence, Charge, Modifications, Score, collision_energy) %>%
    distinct()

fwrite(tbl_qc_ce, ce_output_path, append = TRUE)
